...
const shows = ref([]);
let homeDefaultSearchValue = 'breaking';
const searchInput = ref('');

onMounted(async () => {
  // const res = await fetch(`${config.apiBase}/schedule/web?`);

  const res = await fetch(
    `${config.apiBase}/search/shows?q=${homeDefaultSearchValue}`
  );
...
<template>
  <div>
    <header class="bg-white py-4 shadow-lg">
      ...
      <div
        class="flex p-4 text-sm text-blue-700 bg-blue-100 rounded-lg dark:bg-blue-200 dark:text-blue-800 mx-20 mt-5"
        role="alert"
      >
        <svg
          aria-hidden="true"
        ...
        <div class="text-sm">
          Default Search :
          <span class="font-medium text-lg"> {{ homeDefaultSearchValue }}</span>
        </div>
      </div>
    </header>
    <main>
